...
document.getElementById("Generate-ten").addEventListener("click", generateTen);
document.getElementById("tenBulletBill").addEventListener("click", untilTen);
document.getElementById("CusNum").addEventListener("click", executeCon);
...
function executeCon() {
    ...
    // Assign vairable
    let cusBox = document.getElementById("customNum").value;

    let error1 = document.getElementById("error1");
    let val = document.getElementById("int").value;

    //Check for empty racer positions
    if (val != 1 && val != 2 && val != 3 && val != 4 && val != 5 && val != 6 && val != 7 && val != 8 && val != 9 && val != 10 && val != 11 && val != 12) {
        ...
        // While loop
        let customNumberBox = 0;

        while (customNumberBox < cusBox) {
            generate();
            customNumberBox++;
        }
    }
